...
#Si ils s'arrêtent, alors remettre le chrono a zero et rajoute 1 à la valeur suivante, donc, seconde = min = heure, ainsi de suite
#Ensuite on affichent le résultats et on formate  pour que la largeur s'arrête à 2 chiffres, par défaut 0
                time.sleep(1)
                if(seconde == 60):
                        seconde = 0
                        min+=1
